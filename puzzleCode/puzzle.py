try:
    # Reading  csvFile from the folder
    def readCsvFile(filename):
        readCsvFile = open(filename, 'r')
        menuRead = readCsvFile.read()
        readMenuItem = menuRead.split("\n")
        item_string = readMenuItem[0:len(readMenuItem)]
        return item_string


    csv_item_string = readCsvFile('menu.csv')


    # Function Converting the CSV items in List
    def csvToItemList(csv_item_string):
        csvItemList = []
        for item in csv_item_string:
            int_fields = []
    ...
    # Function declared for giving recommendations
    ...
    # Function declared to help in recommendation
    def helper(listItem, start, items, target):
        ...
        for i in range(start, len(listItem)):
            ...
            # helper(listItem, i+1, items, round(float(target - float(listItem[i][1])), 2))
    ...
    # or if no items are in combination then a message will be displayed
    ...
    print("Please check Error Message" + "\n" + "Error Message :" + str(e))
